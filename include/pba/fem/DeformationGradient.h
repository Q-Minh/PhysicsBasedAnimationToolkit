    ...
    Eigen::MatrixBase<TDerivedX> const& X)
{
    ...
    auto constexpr InputDims                     = TElement::kDims;
    auto constexpr OutputDims                    = TDerivedX::RowsAtCompileTime;
    using AffineElementType                      = typename TElement::AffineBaseType;
    Matrix<TElement::kNodes, InputDims> const GN = TElement::GradN(Xi);
    Matrix<InputDims, OutputDims> const JT       = Jacobian<AffineElementType>(Xi, X).transpose();
    auto JinvT = JT.jacobiSvd(Eigen::ComputeFullU | Eigen::ComputeFullV);
    Matrix<TElement::kNodes, OutputDims> const GP = JinvT.solve(GN.transpose()).transpose();
    return GP;
}
