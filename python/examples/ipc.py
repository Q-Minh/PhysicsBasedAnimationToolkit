...
class HyperElasticPotential:
    ...
    def eval(self, x: np.ndarray):
        return pbat.fem.hyper_elastic_potential(
            ...
            self.GNeg,
            np.ravel(self.mug),
            np.ravel(self.lambdag),
            np.ravel(x),
            self.energy,
            int(pbat.fem.ElementElasticityComputationFlags.Potential),
            self.spd_correction,
            self.element,
            self.order,
            self.dims,
    ...
    def grad(self, x: np.ndarray):
        return pbat.fem.hyper_elastic_potential(
            ...
            np.ravel(x),
            self.energy,
            int(pbat.fem.ElementElasticityComputationFlags.Gradient),
            self.spd_correction,
            self.element,
            self.order,
            self.dims,
    ...
    def hessian(self, x: np.ndarray):
        return pbat.fem.hyper_elastic_potential(
            ...
            np.ravel(x),
            self.energy,
            int(pbat.fem.ElementElasticityComputationFlags.Hessian),
            self.spd_correction,
            self.element,
            self.order,
            self.dims,
