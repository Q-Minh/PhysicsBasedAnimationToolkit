...
def define_args():
    ...
    parser.add_argument(
        ...
        "--input",
        type=str,
        default="/home/mims/Documents/0-PhD/data/cephalopods/octopus-sphere.obj",
...
def remesh(V,T, args):
    ...
    Vr, Tr = gpt.remesh_botsch(V, T, i=args.iterations,
                              project=args.project, h=args.remesh_edge_length,
...
def set_volume_mesh(nodes, elem, attrib):
    # Show determinant
    jac = pbat.fem.determinant_of_jacobian(elem.T, nodes.T, pbat.fem.Element.Tetrahedron, order=1, quadrature_order=1)
...
if __name__ == "__main__":
    ...
    if not args.visual:
        ...
        omesh = meshio.Mesh(nodes, [("tetra", elem)], cell_data={'medit:ref': [attrib.ravel()]})
    ...
    def callback():
        ...
        if args.mode == "cyl":
            a, args.equator_divisions = imgui.InputInt("Equator divisions", args.equator_divisions)
            b, args.height_divisions = imgui.InputInt("Height divisions", args.height_divisions)
            c, args.dims = imgui.InputFloat3("Dimensions", args.dims)
            d, args.normalize = imgui.Checkbox("Normalize", args.normalize)
            if (a or b or c or d) and args.height_divisions > 2 and args.equator_divisions > 3:
            ...
            c, args.dims = imgui.InputFloat3("Dimensions", args.dims)
            d, args.normalize = imgui.Checkbox("Normalize", args.normalize)
            if (a or c or d) and args.resolution[0]>2 and args.resolution[1]>2 and args.resolution[2]>2:
                transform = sm.get_transform()
                V, T = make_prism(
                    resolution=args.resolution,
                    dims=args.dims,
        ...
        if imgui.TreeNode("Remeshing Parameters"):
            a, args.remesh_edge_length = imgui.InputFloat("Target edge length", args.remesh_edge_length)
