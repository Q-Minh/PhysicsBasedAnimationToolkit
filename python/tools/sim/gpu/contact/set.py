...
class ContactSetData:
    ...
    nxx: wp.array[wp.uint32]  # (# points + 1,) prefix sum of point-point contacts
    nxe: wp.array[wp.uint32]  # (# points + 1,) prefix sum of point-edge contacts
    nxf: wp.array[wp.uint32]  # (# points + 1,) prefix sum of point-face contacts
    nee: wp.array[
        wp.uint32
    ...
    xf: wp.array[
        wp.uint64
    ]  # (# point-face contacts capacity,) pairs (i, f) stored as f
    ee: wp.array[
        wp.uint64
...
class ContactSet:
    ...
    def __init__(
        ...
        self._data.xx = wp.zeros((max_contact_pairs[0],), dtype=wp.uint64)
        self._data.xe = wp.zeros((max_contact_pairs[1],), dtype=wp.uint64)
        self._data.xf = wp.zeros((max_contact_pairs[2],), dtype=wp.uint64)
        self._data.ee = wp.zeros((max_contact_pairs[3],), dtype=wp.uint64)
