...
def rest_pose_hyper_elastic_modes(
    ...
    Neg = _fem.shape_functions(n_elements=E.shape[1], element=element, order=order, quadrature_order=qorderM, dtype=wgM.dtype)
    M = _fem.mass_matrix(
        E, 
        X.shape[1], 
        eg=np.flatten(egM), 
        wg=np.flatten(wgM), 
        rhog=np.flatten(rhog), 
        Neg=Neg, 
        dims=X.shape[0], 
        element=element, 
        order=order, 
    ...
    qorderU = order
    wgU = _fem.mesh_quadrature_weights(E, X, element, order=order, quadrature_order=qorderU)
    egU = _fem.mesh_quadrature_elements(E, wgU)
    GNegU = _fem.shape_function_gradients(E, X, element=element, order=order, dims=X.shape[0], quadrature_order=qorderU)
