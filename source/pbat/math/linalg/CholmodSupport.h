 ...
 * @brief Cholmod support for Eigen
 *
 * We copy the relevant parts of Eigen's CholmodSupport module here to avoid
 * having to depend on a new version of Eigen, which does not yet exploit Cholmod's
 * single precision support. This also lets us extend Cholmod support to factorization
 * updates/downdates in the future.
 *
 * @version 0.1
 * @date 2025-10-14
 *
...
// NOTE: Ensure we always include version of CholmodSupport.h first, before any other Eigen/CholmodSupport.
...
template <>
struct cholmod_configure_matrix<float>
{
    template <typename CholmodType>
    static void run(CholmodType& mat)
    {
...
template <>
struct cholmod_configure_matrix<std::complex<float>>
{
    template <typename CholmodType>
    static void run(CholmodType& mat)
    {
...
cholmod_sparse
viewAsCholmod(const SparseSelfAdjointView<const SparseMatrix<Scalar_, Options_, Index_>, UpLo>& mat)
{
    cholmod_sparse res = viewAsCholmod(
        Ref<SparseMatrix<Scalar_, Options_, Index_>>(mat.matrix().const_cast_derived()));
    ...
    EIGEN_STATIC_ASSERT(
        (Options_ & RowMajorBit) == 0 || NumTraits<Scalar_>::IsComplex == 0,
        THIS_METHOD_IS_ONLY_FOR_COLUMN_MAJOR_MATRICES);
    if (Options_ & RowMajorBit)
        res.stype *= -1;
...
Map<const SparseMatrix<Scalar, ColMajor, StorageIndex>> viewAsEigen(cholmod_factor& cm)
{
    return Map<const SparseMatrix<Scalar, ColMajor, StorageIndex>>(
        cm.n,
        cm.n,
        static_cast<StorageIndex*>(cm.p)[cm.n],
        static_cast<StorageIndex*>(cm.p),
        static_cast<StorageIndex*>(cm.i),
        static_cast<Scalar*>(cm.x));
}
...
class CholmodBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    void factorize(const MatrixType& matrix)
    {
        ...
        // If the factorization failed, either the input matrix was zero (so m_cholmodFactor ==
        // nullptr), or minor is the column at which it failed. On success minor == n.
        this->m_info =
            (m_cholmodFactor != nullptr && m_cholmodFactor->minor == m_cholmodFactor->n ?
                 Success :
                 NumericalIssue);
    ...
    void _solve_impl(const MatrixBase<Rhs>& b, MatrixBase<Dest>& dest) const
    {
        eigen_assert(
            m_factorizationIsOk &&
            "The decomposition is not in a valid state for solving, you must first call either "
            "compute() or "
            "symbolic()/numeric()");
        ...
        // Cholmod needs column-major storage without inner-stride, which corresponds to the default
        // behavior of Ref.
        Ref<const Matrix<typename Rhs::Scalar, Dynamic, Dynamic, ColMajor>> b_ref(b.derived());

        cholmod_dense b_cd = viewAsCholmod(b_ref);
        cholmod_dense* x_cd =
            internal::cm_solve<StorageIndex>(CHOLMOD_A, *m_cholmodFactor, b_cd, m_cholmod);
        if (!x_cd)
        {
        ...
        dest = Matrix<Scalar, Dest::RowsAtCompileTime, Dest::ColsAtCompileTime>::Map(
            reinterpret_cast<Scalar*>(x_cd->x),
            b.rows(),
            b.cols());
    ...
    void
    _solve_impl(const SparseMatrixBase<RhsDerived>& b, SparseMatrixBase<DestDerived>& dest) const
    {
        eigen_assert(
            m_factorizationIsOk &&
            "The decomposition is not in a valid state for solving, you must first call either "
        ...
        Ref<SparseMatrix<typename RhsDerived::Scalar, ColMajor, typename RhsDerived::StorageIndex>>
            b_ref(b.const_cast_derived());
        cholmod_sparse b_cs = viewAsCholmod(b_ref);
        cholmod_sparse* x_cs =
            internal::cm_spsolve<StorageIndex>(CHOLMOD_A, *m_cholmodFactor, b_cs, m_cholmod);
        if (!x_cs)
        {
            this->m_info = NumericalIssue;
            return;
        ...
        // TODO optimize this copy by swapping when possible (be careful with alignment, etc.)
        // NOTE cholmod_spsolve in fact just calls the dense solver for blocks of 4 columns at a
        // time (similar to Eigen's sparse solver)
        dest.derived() =
            viewAsEigen<typename DestDerived::Scalar, typename DestDerived::StorageIndex>(*x_cs);
