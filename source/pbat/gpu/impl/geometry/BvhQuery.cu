...
void BvhQuery::DetectOverlaps(
    ...
    [[maybe_unused]] Simplices const& S1,
    [[maybe_unused]] Simplices const& S2,
    [[maybe_unused]] Bvh const& bvh)
{
    ...
    // }
    // overlaps.Clear();
    // auto const leafBegin = bvh.inds.Size() - 1;
    // thrust::for_each(
    //     thrust::device,
    //     thrust::make_counting_iterator(0),
...
void BvhQuery::DetectContactPairsFromOverlaps(
    ...
    [[maybe_unused]] Simplices const& S1,
    [[maybe_unused]] Simplices const& S2,
    [[maybe_unused]] Bodies const& BV,
    [[maybe_unused]] Bvh const& bvh,
    [[maybe_unused]] GpuScalar dhat,
    [[maybe_unused]] GpuScalar dzero)
{
...
TEST_CASE("[gpu][impl][geometry] BvhQuery")
{
    ...
    // // Duplicate mesh into 2 bodies and translate 2nd mesh to get a single overlapping vertex
    // (15)
    ...
    // GpuIndexMatrixX V = GpuIndexVectorX::LinSpaced(P.cols(), 0, static_cast<GpuIndex>(P.cols() -
    // 1))
    //                         .reshaped(1, P.cols());
    ...
    // bool const bNearestVerticesFound = (nearestNeighbours[0].first == expectedNearestVertex[0]
    // and
    //                                     nearestNeighbours[1].first == expectedNearestVertex[1])
    //                                     or
    //                                    (nearestNeighbours[0].first == expectedNearestVertex[1]
    //                                    and
    //                                     nearestNeighbours[1].first == expectedNearestVertex[0]);
    // CHECK(bNearestVerticesFound);
    // for (auto const& [v, f] : nearestNeighbours)
    // {
    //     // Nearest neighbour contact pairs should not come from the same body, i.e. self-contact
    //     is
