...
struct VectorView
{
    ...
    T* data; ///< Pointer to the vector coefficients
    ...
    // Accessors
    VectorView<T> Slice(auto row, auto rows, auto inc) const
    {
        return VectorView<T>{data + row, rows, inc};
    }
    VectorView<T> Segment(auto row, auto rows) const { return Slice(row, rows, 1); }
    VectorView<T> Head(auto rows) const { return Slice(0, rows, 1); }
    VectorView<T> Tail(auto rows) const { return Slice(n - rows, rows, 1); }
};
...
struct MatrixView
{
    ...
    T* data;              ///< Pointer to the matrix coefficients
    ...
    // Accessors
    MatrixView<T> Transposed() const
    {
        return MatrixView<T>{data, m, n, ld, op == CUBLAS_OP_N ? CUBLAS_OP_T : CUBLAS_OP_N};
    }
    MatrixView<T> SubMatrix(auto row, auto col, auto rows, auto cols) const
    {
        return MatrixView<T>{data + ld * col + row, rows, cols, ld};
    }
    MatrixView<T> LeftCols(auto cols) const { return SubMatrix(0, 0, m, cols); }
    MatrixView<T> RightCols(auto cols) const { return SubMatrix(0, Cols() - cols, m, cols); }
    MatrixView<T> TopRows(auto rows) const { return SubMatrix(0, 0, rows, Cols()); }
    MatrixView<T> BottomRows(auto rows) const { return SubMatrix(Rows() - rows, 0, rows, Cols()); }
    MatrixView<T> Col(auto col) const { return SubMatrix(0, col, Rows(), Cols()); }
    MatrixView<T> Row(auto row) const { return SubMatrix(row, 0, 1, Cols()); }
    VectorView<T> Flattened() const { return VectorView<T>{data, Rows() * Cols(), 1}; }
};
...
struct Matrix
{
    ...
    common::Buffer<T> data; ///< `m x n` dense matrix coefficients in column-major order
    int m;                  ///< Number of rows

    // Storage information
    T* Raw() { return data.Raw(); }
    T const* Raw() const { return data.Raw(); }
    auto Rows() const { return m; }
    ...
    // Accessors
    MatrixView<T> View() const { return MatrixView<T>{data.Raw(), m, Cols(), m}; }
    MatrixView<T> SubMatrix(auto row, auto col, auto rows, auto cols) const
    {
        T* a = data.Raw();
        return MatrixView<T>{a + m * col + row, rows, cols, m};
    }
    MatrixView<T> LeftCols(auto cols) const { return SubMatrix(0, 0, m, cols); }
    MatrixView<T> RightCols(auto cols) const { return SubMatrix(0, Cols() - cols, m, cols); }
    MatrixView<T> TopRows(auto rows) const { return SubMatrix(0, 0, rows, Cols()); }
    ...
    MatrixView<T> Col(auto col) const { return SubMatrix(0, col, Rows(), Cols()); }
    MatrixView<T> Row(auto row) const { return SubMatrix(row, 0, 1, Cols()); }
    VectorView<T> Flattened() const { return VectorView<T>{data.Raw(), Rows() * Cols(), 1}; }
};
...
struct Vector
{
    ...
    common::Buffer<T> data; ///< `n x 1` dense vector coefficients
    int n;                  ///< Number of rows
    ...
    {
        T* a = data.Raw();
        return VectorView<T>{a + row, rows, inc};
    }
    VectorView<T> Segment(auto row, auto rows) const { return Slice(row, rows, 1); }
    VectorView<T> Head(auto rows) const { return Slice(0, rows, 1); }
    VectorView<T> Tail(auto rows) const { return Slice(n - rows, rows, 1); }
