...
AndersonIntegrator::AndersonIntegrator(Data const& data)
    : Integrator(data),
      ...
      Gkm1(data.X.size()),
      xkm1(data.X.size()),
      mDFK(data.X.size(), data.mAndersonWindowSize),
      mDGK(data.X.size(), data.mAndersonWindowSize),
      mQR(data.X.size(), data.mAndersonWindowSize),
      mTau(data.mAndersonWindowSize),
      mBlas(),
      mLinearSolver(),
