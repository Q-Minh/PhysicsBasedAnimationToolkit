...
class Integrator
{
  public:
    ...
    using CollisionCandidateType = typename impl::geometry::BvhQuery::OverlapType;
    using ContactPairType        = typename impl::geometry::BvhQuery::NearestNeighbourPairType;
  ...
  public:
    geometry::Points X;    ///< Vertex/particle positions
    geometry::Simplices V; ///< Boundary vertex simplices
    geometry::Simplices F; ///< Boundary triangle simplices
    geometry::Simplices T; ///< Tetrahedral simplices

    geometry::Bodies BV; ///< Bodies of particles
  private:
    geometry::Bvh Tbvh;        ///< Tetrahedron bvh
    geometry::Bvh Fbvh;        ///< Triangle bvh (over boundary mesh)
    geometry::BvhQuery Vquery; ///< BVH vertex queries
