 ...
 * @brief Compute linear tangential operator \f$ \mathbf{T} \f$ for edge-edge contact s.t.
 * tangential displacements are \f$ \mathbf{T} \begin{bmatrix}\mathbf{p_1} \\ \mathbf{q_1}
 * \\ \mathbf{p_2} \\ \mathbf{q_2}\end{bmatrix} \f$, where the closest points `x,y` on both edges
 * are pre-computed and passed in along with their barycentric coordinates `u1` and `u2`.
 *
 * This is a convenience overload that takes the closest points `x,y` directly, rather than
 * requiring the edge endpoints. The tangent basis is computed using `PointPointTangentialBasis`.
 *
 * @tparam TMatrixX Matrix type for closest point on first edge
 * @tparam TMatrixY Matrix type for closest point on second edge
 * @tparam TScalar Scalar type
 * @param x `3 x 1` closest point on first edge (x = (1-u1)*p1 + u1*q1)
 * @param y `3 x 1` closest point on second edge (y = (1-u2)*p2 + u2*q2)
 * @param u1 Barycentric coordinate of closest point on first edge
 * @param u2 Barycentric coordinate of closest point on second edge
 * @param eps Numerical tolerance for degenerate case handling
 * @return `4*|# dims| x 2` tangential operator
 * @pre `0 < u1 < 1 and 0 < u2 < 1`
 */
template <
    math::linalg::mini::CMatrix TMatrixX,
    math::linalg::mini::CMatrix TMatrixY,
    ...
    TScalar eps = std::numeric_limits<TScalar>::epsilon())
{
    ...
    assert(u2 >= TScalar(0) and u2 <= TScalar(1));
    auto constexpr kDims         = TMatrixX::kRows;
    SMatrix<TScalar, kDims, 2> B = PointPointTangentialBasis(x, y, eps);
    SMatrix<TScalar, 4 * kDims, 2> T;
    T.template Slice<kDims, 2>(0, 0)         = (TScalar(1) - u1) * B;
    T.template Slice<kDims, 2>(kDims, 0)     = u1 * B;
    T.template Slice<kDims, 2>(2 * kDims, 0) = (u2 - TScalar(1)) * B;
...
/**
 * @brief Compute the (i-th) block of the linear tangential operator \f$ \mathbf{T} \f$ returned by
 * the overloaded `EdgeEdgeLinearTangentialOperator` that takes closest points `x,y`.
 *
 * This is a convenience overload that takes the closest points `x,y` directly, rather than
 * requiring the edge endpoints. The tangent basis is computed using `PointPointTangentialBasis`.
 *
